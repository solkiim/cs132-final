...
exports.getfavoritestocks = function(pool, req, res) {
	pool.query('SELECT tokenSymbol FROM user_favorite_stock WHERE username=$1',
		[req.body.username],
		function(err, data) {
			if (err) {
				console.error(err);
				return res.status(500).send('error getting favorite stocks');
			}
			res.json(data.rows.map(stock => stock.tokenSymbol));
		}
	);
}

// add new favorite stock
exports.addfavoritestock = function(pool, req, res) {
	pool.query(
		'INSERT INTO user_favorite_stock VALUES ($1, $2)',
		[req.body.username, req.body.tokenSymbol],
		function(err, data) {
			if (err) {
				console.error(err);
				return res.status(500).send('error adding favorite stock');
			}
			res.sendStatus(204);
		}
	);
...
exports.removefavoritestock = function(pool, req, res) {
	pool.query(
		'DELETE FROM user_favorite_stock VALUES WHERE username=$1 AND tokenSymbol=$2',
		[req.body.username, req.body.tokenSymbol],
		function(err, data) {
			if (err) {
				console.error(err);
				return res.status(500).send('error removing favorite stock');
			}
			res.sendStatus(204);
		}
	);
...
exports.getportfolio = function(pool, req, res) {
	pool.query('SELECT tokenSymbol FROM user_portfolio WHERE username=$1',
		[req.body.username],
		function(err, data) {
			if (err) {
				console.error(err);
...
exports.addtoportfolio = function(pool, req, res) {
	pool.query(
		'INSERT INTO user_portfolio VALUES ($1, $2)',
		[req.body.username, req.body.tokenSymbol],
		function(err, data) {
			if (err) {
				console.error(err);
				return res.status(500).send('error adding to portfolio');
			}
			res.sendStatus(204);
		}
	);
...
exports.removefromportfolio = function(pool, req, res) {
	pool.query(
		'DELETE FROM user_portfolio VALUES WHERE username=$1 AND tokenSymbol=$2',
		[req.body.username, req.body.tokenSymbol],
		function(err, data) {
			if (err) {
				console.error(err);
				return res.status(500).send('error removing from portfolio');
			}
			res.sendStatus(204);
		}
	);
